...
export default function Dashboard() {
  ...
  const [loadError, setLoadError]           = useState(false);       // ① loadError
  const [activeTab, setActiveTab]           = useState<'predict' | 'my' | 'leaders' | 'feed' | 'history'>('predict');
  const [activeRound, setActiveRound]       = useState('');                         // ② smart activeRound
  ...
  const loadData = async (userId: string) => {
    setLoadError(false); // reset on retry
    try {
      ...
      if (availableRounds.length > 0) {
        // ② activeRound ذكي — أول round فيها is_open=true و actual_home_score=null
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 16px' }}>
        ...
        {activeTab === 'leaders' && (
          <div>
            ...
            ) : leaderboard.slice(0, 20).map((player: any, i) => {   // ⑤ slice(0,20)
