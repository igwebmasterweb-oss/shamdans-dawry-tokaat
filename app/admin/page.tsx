...
export default function AdminPage() {
  ...
  // ✅ الجديد — بدل surpriseQ/surpriseA
  ...
  // ✅ loadMatches — بدل surprise_* أضفنا الـ 3 حقول الجديدة
  ...
  // ✅ باقي الـ load functions محفوظة بالظبط زي الأصل
  ...
  // ✅ silentUpdateResults — محفوظ بالظبط
  ...
  // ✅ openResultModal — بدل setSurpriseQ/A أضفنا الـ 3 checkboxes
  ...
  // ✅ saveResult — بدل surprise_* أضفنا الـ 3 حقول + update-results فوراً (محفوظ)
  const saveResult = async () => {
    ...
    try {
      ...
      // ✅ محفوظ — تشغيل update-results فوراً
      const res  = await fetch('/api/update-results');
      const data = await res.json();
  ...
  // ✅ savingResult state — لازم يكون موجود
  ...
  return (
    <>
      ...
      {/* STATS */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:12,padding:'20px 24px'}}>
        {[
          ...
          <div key={s.label} style={{background:'var(--surface)',border:'1px solid var(--line)',borderRadius:16,padding:'14px 16px',textAlign:'center'}}>
            <div style={{fontSize:11,color:'var(--muted)',marginBottom:6,fontWeight:700}}>{s.label}</div>
            <div style={{fontSize:26,fontWeight:900,color:s.color,fontVariantNumeric:'tabular-nums'}}>{s.value}</div>
          </div>
        ))}
      </div>
                ...
                <div key={match.fixture.id} style={{background:'var(--surface)',border:'1px solid var(--line)',borderRadius:18,padding:'16px 20px',marginBottom:10,display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
                  <div style={{flex:1,minWidth:200}}>
                    ...
                    <div style={{fontSize:12,color:'var(--muted)',display:'flex',gap:12,flexWrap:'wrap'}}>
                      ...
                      {matchPreds.length>0 && <span style={{color:'#38bdf8'}}>👥 {matchPreds.length} توقع</span>}
                ...
                <div key={lg.id} className="league-card">
                  <div style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:12,color:'var(--muted)',display:'flex',gap:12,flexWrap:'wrap'}}>
                        <span>كود: <strong style={{color:'var(--text)'}}>{lg.code}</strong></span>
      ...
      {showModal && selectedMatch && (
        <div onClick={()=>setShowModal(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.7)',backdropFilter:'blur(6px)',display:'grid',placeItems:'center',zIndex:1000,padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'var(--surface)',border:'1px solid var(--line)',borderRadius:24,padding:28,width:'100%',maxWidth:480,maxHeight:'90vh',overflowY:'auto'}}>
            ...
            {/* ✅ Checkboxes — وقت إضافي + الـ 3 الجديدة */}
