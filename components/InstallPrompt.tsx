...
export default function InstallPrompt() {
  ...
  const [isStandalone, setIsStandalone] = useState(false); // ✅ false بدل true
    ...
    <>
      <style>{`
        .pwa-banner {
          ...
          border: 1px solid rgba(217,178,95,.35); border-radius: 18px;
          padding: 16px 18px; display: flex; align-items: center; gap: 12px;
          max-width: 370px; width: calc(100% - 32px);
          box-shadow: 0 8px 32px rgba(0,0,0,.6); direction: rtl;
          font-family: 'Cairo', sans-serif; animation: slideUp .35s ease;
        }
        @keyframes slideUp {
          from { opacity: 0; transform: translate(-50%, 20px); }
          to   { opacity: 1; transform: translate(-50%, 0); }
        }
        .pwa-banner img { width: 44px; height: 44px; object-fit: contain; flex-shrink: 0; border-radius: 10px; }
        .pwa-banner-text { flex: 1; }
        .pwa-banner-title { font-size: 14px; font-weight: 800; color: #f4f1e8; margin-bottom: 3px; }
        .pwa-banner-sub { font-size: 12px; color: #a8a39a; line-height: 1.6; }
        .pwa-banner-sub strong { color: #d9b25f; }
        .pwa-install-btn {
          background: linear-gradient(135deg,#d9b25f,#a8761a); color: #0a0800;
